...
function Position() {
  ...
  useEffect(() => {
    axios
      .get<Position[]>("http://localhost:3001/get/position")
      .then((res) => {
        setData(res.data);
      })
  ...
  const handleDelete = (id: string) => {
    axios
      .delete(`http://localhost:3001/deletePositions/${id}`)
      .then((res) => {
        console.log(res);
        // Обновляем данные после удаления
        setData((prevData) => prevData.filter((item) => item._id !== id));
        setFilteredPosition((prevFilteredPosition) =>
          prevFilteredPosition.filter((item) => item._id !== id)
        );
      })
      .catch((err) => console.log(err));
  };
                  ...
                  <tr key={index}>
                    ...
                    <td className="td-icon">
                      <div className={"icon_edit"}>
                        <Link to={`/position/updatePosition/${position._id}`}>
                          <FiEdit />
                        </Link>
                      </div>
                    </td>
