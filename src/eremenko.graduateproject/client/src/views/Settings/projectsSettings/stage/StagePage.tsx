...
function StagePage() {
  ...
  useEffect(() => {
    axios
      .get<Stage[]>("http://localhost:3001/get/stage")
      .then((res) => {
        setData(res.data);
      })
  ...
  const handleDelete = (id: string) => {
    axios
      .delete(`http://localhost:3001/delete/stage/${id}`)
      .then((res) => {
        console.log(res);
        // Обновляем данные после удаления
        setData((prevData) => prevData.filter((item) => item._id !== id));
        setFilteredStage((prevFilteredStage) =>
          prevFilteredStage.filter((item) => item._id !== id)
        );
      })
      .catch((err) => console.log(err));
  };
                  ...
                  <tr key={index}>
                    ...
                    <td className="td-icon">
                      <div className={"icon_edit"}>
                        <Link to={`/stagePage/updateStage/${stage._id}`}>
                          <FiEdit />
                        </Link>
                      </div>
                    </td>
