...
function SoftwareProductDevelopment() {
    ...
    const fetchStages = () => {
        axios.get<Stage[]>(`http://localhost:3001/projects/${projectId}/SPDStage`)
            .then(res => {
                setStages(res.data);
            })
    ...
    const handleDeleteStage = (stageId: string) => {
        axios.delete(`http://localhost:3001/projects/${projectId}/SPDStage/${stageId}`)
            .then(res => {
                fetchStages();
            })
            .catch(err => console.log(err));
    ...
    const handleDeleteTask = (stageId: string, taskId: string) => {
        axios.delete(`http://localhost:3001/projects/${projectId}/SPDStage/${stageId}/tasks/${taskId}`)
            .then(res => {
                fetchStages();
            })
            .catch(err => console.log(err));
    ...
    const handleTaskMove = (task: Task, targetStageId: string) => {
        ...
        axios.post(`http://localhost:3001/projects/${projectId}/SPDStage/${sourceStageId}/tasks/${task._id}/move/${targetStageId}`)
            .then(res => {
                fetchStages();
            })
            .catch(err => console.log(err));
            ...
            <div className='container'>
                <div className={'container_search_filter'}>
                    ...
                    <div className={'containet_btn_add'}>
                        <Link to={`/projectsPage/projectDetails/${projectId}/addSPDPage`}>
                            <button className={'btn_add_stage'}>Добавить статус</button>
                        </Link>
                    </div>
                </div>
                <div><p>Разработака программного продукта</p></div>
            </div>
            <div className='containet_btn_add_task'>
                {stages.length > 0 && (
                    <Link to={`/projectsPage/projectDetails/${projectId}/addTaskSPDPage/${stages[0]._id}`}>
