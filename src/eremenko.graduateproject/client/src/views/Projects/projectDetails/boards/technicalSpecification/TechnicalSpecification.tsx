...
function TechnicalSpecification() {
    ...
    const fetchStages = () => {
        axios.get<Stage[]>(`http://localhost:3001/projects/${projectId}/TechnicalSpecificationStage`)
            .then(res => {
                setStages(res.data);
            })
    ...
    // Функция для удаления стадии
    const handleDeleteStage = (stageId: string) => {
        axios.delete(`http://localhost:3001/projects/${projectId}/TechnicalSpecificationStage/${stageId}`)
            .then(res => {
                fetchStages();
            })
            .catch(err => console.log(err));
    ...
    const handleDeleteTask = (stageId: string, taskId: string) => {
        axios.delete(`http://localhost:3001/projects/${projectId}/TechnicalSpecificationStage/${stageId}/tasks/${taskId}`)
            .then(res => {
                fetchStages();
            })
            .catch(err => console.log(err));
    ...
    const handleTaskMove = (task: Task, targetStageId: string) => {
        ...
        axios.post(`http://localhost:3001/projects/${projectId}/TechnicalSpecificationStage/${sourceStageId}/tasks/${task._id}/move/${targetStageId}`)
            .then(res => {
                fetchStages();
            })
            .catch(err => console.log(err));
    ...
    return (
        <>
            ...
            <div className='container'>
                <div className={'container_search_filter'}>
                    ...
                    <div className={'containet_btn_add'}>
                        <Link to={`/projectsPage/projectDetails/${projectId}/TechnicalSpecificationStage`}>
            ...
            <div className='containet_btn_add_task'>
                {stages.length > 0 && (
                    <Link to={`/projectsPage/projectDetails/${projectId}/addTaskTechnicalSpecificationPage/${stages[0]._id}`}>
