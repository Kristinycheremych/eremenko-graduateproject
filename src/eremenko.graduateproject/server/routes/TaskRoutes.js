...
router.get('/tasks/:taskStatusId', async (req, res) => {
    try {
        const tasks = await TaskModel.find({ taskStatusId: req.params.taskStatusId }).populate('employees');
        res.json(tasks);
    } catch (error) {
        console.error('Ошибка при получении задач для конкретного статуса задачи:', error);
        res.status(500).json({ error: 'Ошибка сервера при получении задач для конкретного статуса задачи' });
    }
});
...
router.post('/tasks/:taskStatusId', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Ошибка сервера при создании задачи для конкретного статуса' });
    }
});
...
router.put('/tasks/:taskId', async (req, res) => {
    try {
        ...
        console.error('Ошибка при изменении задачи для конкретного статуса:', error);
        res.status(500).json({ error: 'Ошибка сервера при изменении задачи для конкретного статуса' });
    }
});
...
router.delete('/tasks/:taskId', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Ошибка сервера при удалении задачи для конкретного статуса' });
    }
});
...
router.post('/tasks/:taskId/move/:newTaskStatusId', async (req, res) => {
    try {
        ...
        const updatedTask = await TaskModel.findByIdAndUpdate(taskId, { taskStatusId: newTaskStatusId }, { new: true }).populate('employees');
